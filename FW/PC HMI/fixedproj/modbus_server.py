...
def run_modbus_process(runtime: dict) -> None:
    ...
    try:
        ser = serial.Serial(
            ...
            parity=str(cfg.get("parity", "N")).upper(),
            stopbits=int(cfg.get("stopbits", 1)),
            timeout=float(cfg.get("timeout", 0.0015)),
            write_timeout=float(cfg.get("write_timeout", 0.02)),
            inter_byte_timeout=float(cfg.get("inter_byte_timeout", 0.0015)),
            exclusive=False,
        )
